		...
		app.factory('httpInterceptor', ['$q','$injector',function($q, $injector) {
		    ...
		    var httpInterceptor = {
		      request : function(config) {//请求
				if (config.url.indexOf(sysConfig.BaseUrl[sysConfig.ApiEnv]>-1||sysConfig.BaseZSTJUrl[sysConfig.ZSTJApiEnv])>-1){
		      ...
		      response : function(response) {//请求接收成功，响应				
		      	if (response.config.url.indexOf(sysConfig.BaseUrl[sysConfig.ApiEnv]>-1||sysConfig.BaseZSTJUrl[sysConfig.ZSTJApiEnv])>-1){
		      		penddingRequestCount--;
		      		if(penddingRequestCount==0){
		      			util.hideAjaxLoading();
		      		}
