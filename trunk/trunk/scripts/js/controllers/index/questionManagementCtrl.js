...
define(['indexApp'
      ...
      , 'model/questionManagementModel']
      , function (app, _, Extend, Const, util, Validation, cacheUser, ViewModel) {
  app.controller('questionManagementCtrl', ['$scope'
                                          , '$http'
                                          , '$routeParams'
                                          , '$window'
    ...
    var ServerUrls = {
      ...
      EvaluationResult: {
        // 添加问卷评测结果
        // api/Question/AddQuestionnaireCalculation
        Add: 'Question/AddQuestionnaireCalculation',
        // 获取问卷评测结果列表
        // api/GetQuestionnaireCalculations/{questionnaireId}
        List: 'GetQuestionnaireCalculations/{0}',
        // 删除问卷评测结果
        // api/Question/DelQuestionnaireCalculation?id={id}
        Delete: 'Question/DelQuestionnaireCalculation?id={0}'
      },
      Preview: {      // 预览问卷的URL
      },
      Public: {       // 发布问卷的URL
    ...
    $scope.vm = {
      ...
      QuestionCollection: {
        ...
        code: '',                           // （问卷编号）
        desc: '',                           // （问卷说明）
        typeData: Const.QuestionTypeData,   // （问卷分类集合）
        subjectCollection: {
          data: [      // {subject}
            //{
            //    id, // （编号）
          ...
          if (qId > 0) {
            ...
            util.ajaxGet($http, url, function (result) {
              ...
              if (result.state == 1) {
                ...
                question.type = entity.IsCalc ? 1 : 0;
                ...
                if(question.type == 1)
                  $scope.vm.EvaluationResult.Init();
              }
              else {
      ...
      EditQuestion: {                       // 编辑问卷
        ...
        OnSubmit: function () {
          var tabs = $scope.vm.TabsCollection,
            question = $scope.vm.QuestionCollection,
            evaluation = $scope.vm.EvaluationResult,
            ...
            evaluationPage = 4,
            // type 1: 评测类型 2: 非评测类型
            nextPage = question.type == 1 ? evaluationPage: previewPage,
      ...
      EvaluationResult: {                   // 评测结果及建议
        ...
        PrevPage: function(){
          var tabs = $scope.vm.TabsCollection,
              question = $scope.vm.QuestionCollection,
              questionItemPage = 1;  // 添加题目选项

          // 跳转下一页面
          tabs.CurrentPage(questionItemPage);
        },
        ShowNextPage: function(){
          var self = $scope.vm.EvaluationResult;
          if (self.List.length) {
            self.NextPage();
          }else{
            $('#panelEvaluationConfrom').modal('show');
          }
        },
        NextPage:　function(){
          var tabs = $scope.vm.TabsCollection,
              ...
              previewPage = 2;  // 预览页面
          // 跳转下一页面
          tabs.CurrentPage(previewPage);
        },
        OnLoadList: function(){
          var self = $scope.vm.EvaluationResult,
              question = $scope.vm.QuestionCollection,
              url = String.prototype.format(ServerUrls.EvaluationResult.List, question.id),
              vm = new ViewModel();
          ...
          util.ajaxGet($http, url, function (result) {
              ...
              if (result.state == 1) {
                if(result.Data)
                {
                  var tempList = vm.convertEvaluationResult(result.Data);
                  self.List = tempList;
                } else {
                  self.List = [];
                }
        ...
        ValidEvaluation: function (param) {
          var self = $scope.vm.EvaluationResult,
              valid = new Validation();
            valid.add(param.LowScore, 'integer', '请录入正确的最低分值！');
            valid.add(param.HignScore, 'integer', '请录入正确的最高分值！');
            // valid.add([param.LowScore, param.HignScore], 'noLessThan', '请录入不小于最低分值的数字');
            valid.add(param.CalcResult, 'maxLength:200', '评测结果最多只能包含200个字符！');
            valid.add(param.CalcSuggest, 'maxLength:2000', '评测建议最多只能包含2000个字符！');
          ...
          for (var i = 0; i < self.List.length; i++) {
            ...
            if (item.LowScore <= param.LowScore && item.HignScore >= param.LowScore) {
              util.showFade('已存在指定范围的分值!');
              return false;
            }
            if (item.LowScore <= param.HignScore && item.HignScore >= param.HignScore) {
              util.showFade('已存在指定范围的分值!');
              return false;
            }
          }

          if ((param.HignScore - 0) <= param.LowScore) {
              util.showFade('请录入不小于最低分值的数字!');
              return false;
          }
        ...
        OnAddItem: function(){
          var self = $scope.vm.EvaluationResult,
              question = $scope.vm.QuestionCollection,
              userInfo = cacheUser.UserInfoManager.GetUserInfo(),
              url = ServerUrls.EvaluationResult.Add,
              param = {
                QuestionnaireId: question.id,
                OrderNum: 0,
                LowScore: self.Current.LowScore,
                HignScore: self.Current.HignScore,
                CalcResult: self.Current.CalcResult,
                CalcSuggest: self.Current.CalcSuggest,
                OperateBy: userInfo.Id
              };
            if(self.ValidEvaluation(param)){
              util.ajaxPost($http, url, param, function (result) {
                //console.log(result);
                if (result.state == 1) {
                  util.showFade('添加问卷评测结果成功！');
                  self.OnClear();
                  self.Init();
                } else {
                  util.showFade(result.message);
        ...
        OnRemoveItemDialog: function(item){
          var self = $scope.vm.EvaluationResult;

          self.Current = item;

          $('#delEvaluation').modal('show');
        },
        OnRemoveItem: function(){
          var self = $scope.vm.EvaluationResult,
              url = String.prototype.format(ServerUrls.EvaluationResult.Delete, self.Current.Id),
              param = {};

          util.ajaxPost($http, url, param, function (result) {
              //console.log(result);
              if (result.state == 1) {
                util.showFade('删除评测结果成功！');
                self.Init();
                $('#delEvaluation').modal('hide');
              } else {
                util.showFade(result.message);
              }
            }, function (err) { util.showFade(err); });
      ...
      PreviewQuestion: {                    // 预览问卷
        ...
        PrevPage: function(){
          var tabs = $scope.vm.TabsCollection,
              question = $scope.vm.QuestionCollection,
              previewPage = question.type == 1 ? 4 : 1;  // 预览页面
            ...
            for (var i = 0; i < subjects.viewCount; i++) {
              ...
              if (subjects.viewData[i].id == item.id) {
                if (item.selectAnswerItem) {
                  ...
                  if(item.answerCollection){
                    for(var x1 =0; x1 < item.answerCollection.count; x1++){
                      var current = item.answerCollection.data[x1];
                      if(current.selectedClass){
                        evaluation.PreviewAnswer.push(current);
                        break;
                      }
                    }
                  }
        ...
        Success: function (item) {
          ...
          if (item && item.selectItem) {
            evaluation.PreviewAnswer.push(item.selectAnswerItem);
          }
        ...
        SuccessOK: function () {
          var self = $scope.vm.PreviewQuestion,
              evaluation = $scope.vm.EvaluationResult,
              public = $scope.vm.PublicQuestion;
          //util.showFade('预览成功！！');
          if(evaluation.PreviewAnswer.length && $scope.vm.QuestionCollection.type == 1){
            public.Current = {
                    Score: '',
                    CalcResult: '',
                    CalcSuggest: ''
            ...
            var score = 0;
            for(var i=0; i< evaluation.PreviewAnswer.length; i++){
              var item = evaluation.PreviewAnswer[i];
              score += item.score;
            }
            if(item.answerCollection){
              for(var x1 =0; x1 < item.answerCollection.count; x1++){
            ...
            for(var j=0;j< evaluation.List.length; j++){
              var item = evaluation.List[j];
              if(score >= item.LowScore && score <= item.HignScore){
                public.Current = {
                  Score: score,
                  CalcResult: item.CalcResult,
                  CalcSuggest: item.CalcSuggest
                };
                break;
              }
            }
            if (!public.Current.Score && score > 0) {
              $('#panelEvaluationAlert').modal('show');
            }
          }
